...
SKILL.md keeps an arrow label 6-10px clear of its connector, and fixes the
paint order as background -> zones -> arrows -> labels -> nodes. Nothing keeps
a label mask off a *node*, so a label whose mask lands partly inside a node
...
Usage:
    python3 scripts/verify_geometry.py <file> [<file> ...]
...
def main() -> int:
    ...
    parser.add_argument("--all", action="store_true", help="check every shipped example")
