...
Usage:
    python3 mermaid_extract.py <file> [--fence N] [--out PATH]

stdout: a JSON digest { kind, direction, nodes, edges, containers, hubs, budget }.
stderr: diagnostics.

Exit codes: 0 success | 1 parse failure | 2 usage error | 3 integrity failure.
"""
...
def _parse_fail(message: str) -> NoReturn:
    print(f"mermaid_extract: {message}", file=sys.stderr)
    raise SystemExit(1)
...
class Diagram:
    ...
    def add_node(
        ...
        if len(self.nodes) >= MAX_NODES:
            _integrity_fail(f"node limit exceeded (max {MAX_NODES})")
        node = Node(
            id=node_id,
    ...
    def add_edge(
        ...
        if len(self.edges) >= MAX_EDGES:
            _integrity_fail(f"edge limit exceeded (max {MAX_EDGES})")
        edge = Edge(
...
def load_blocks(path: Path) -> list[SourceBlock]:
    ...
    if start is not None:
        _parse_fail(f"{path.name}: unterminated mermaid fence starting at line {start - 1}")
    if not blocks:
        _integrity_fail(f"{path.name}: no fenced mermaid block found")
    return blocks
...
def _kind_and_direction(
    ...
    for position, (line_number, raw) in enumerate(lines):
        ...
        _parse_fail(f"not a Mermaid file at line {line_number}")
    _integrity_fail("not a Mermaid file")
...
def _logical_statements(
    ...
    for line_number, raw in lines:
        ...
        pending = []
    if pending:
        _parse_fail(f"unterminated statement at line {start_line}")
    return logical
    ...
    for line_number, raw in _logical_statements(lines[header_position + 1 :]):
        ...
        if operators:
            ...
            if len(segments) != len(operators) + 1:
                _parse_fail(f"malformed edge at line {line_number}")
...
def compact_digest(path: Path, diagram: Diagram) -> dict[str, Any]:
    analysis = analyze(diagram)
    containers = [
        {"id": node.id, "label": node.label, "children": list(node.children)}
        for node in diagram.nodes
        if node.container or node.children
    ]
    nodes = [
        {"id": node.id, "label": node.label, "kind": shape_family(node.shape)}
        for node in diagram.nodes
    ]
    edges = [
        {"src": edge.source, "dst": edge.target, "label": edge.label}
        for edge in diagram.edges
    ]
    return {
        "source": str(path),
        ...
        "hubs": analysis["hubs"],
        "budget": {
            "nodes": analysis["nodes_drawable"],
            "over_nine": analysis["over_node_budget"],
        },
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="select the Nth fenced mermaid block (default 0)",
    )
    parser.add_argument("--out", help="write the JSON digest to this path instead of stdout")
    args = parser.parse_args(argv)
    if args.fence < 0:
        _usage("--fence must be >= 0")

    path = Path(args.file)
    if not path.is_file():
        _usage(f"{path}: no such file")
    blocks = load_blocks(path)
    if args.fence >= len(blocks):
        _usage(
            f"--fence {args.fence} out of range (found {len(blocks)} block(s))"
        )
    diagram = parse_block(blocks[args.fence])

    output = json.dumps(compact_digest(path, diagram), indent=2, ensure_ascii=False)
    if args.out:
        try:
            Path(args.out).write_text(output + "\n", encoding="utf-8")
        except OSError as error:
            _usage(f"cannot write {args.out}: {error}")
